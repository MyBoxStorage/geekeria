...
export default function CheckoutPending() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-cosmos/10 via-cosmos/5 to-fire/10 p-4">
        ...
        <CardContent className="p-6 sm:p-8 text-center">
          ...
          <div className="mb-6">
            <div className="relative inline-flex items-center justify-center">
              <div className="absolute inset-0 bg-cosmos/20 rounded-full animate-ping" style={{ animationDuration: '2s' }} />
              <Clock className="w-20 h-20 sm:w-24 sm:h-24 text-cosmos relative z-10 animate-pulse" style={{ animationDuration: '2s' }} />
            </div>
          </div>
          ...
          {/* Subtítulo */}
          <p className="text-base sm:text-lg text-cosmos/70 mb-4 font-body">
            Aguardando confirmação do pagamento.
          </p>

          {/* Instruções claras */}
          <div className="mb-6 p-4 bg-cosmos/5 rounded-xl border border-cosmos/10 text-left">
            <div className="flex items-start gap-2.5">
              <Info className="w-4 h-4 text-cosmos/60 mt-0.5 shrink-0" />
              <div className="space-y-1.5 text-sm text-cosmos/70 font-body leading-relaxed">
          ...
          <div className="mb-6">
            <span className="inline-flex items-center gap-2 bg-cosmos/15 text-cosmos border border-cosmos/30 rounded-full px-4 py-2 text-sm font-medium">
          ...
          {resolvedRef && (
            <div className="bg-gradient-to-r from-cosmos/10 to-cosmos/5 rounded-xl p-5 sm:p-6 mb-6 border border-cosmos/30 hover:shadow-lg transition-shadow duration-300">
              <p className="text-sm font-medium text-cosmos/70 mb-2 font-body">Referencia do pedido</p>
              <p className="text-xl sm:text-2xl font-mono font-bold text-cosmos">
                #{resolvedRef}
              </p>
            </div>
          )}
          ...
          {isPix && pixData && (
            <div className="mb-6 space-y-4">
              ...
              <Alert className="border-cosmos/30 bg-cosmos/5 rounded-xl">
                <QrCode className="h-5 w-5 text-cosmos flex-shrink-0" />
                <AlertDescription className="text-cosmos text-left ml-2 font-body">
              ...
              {pixData.qrCodeBase64 ? (
                <div className="bg-surface p-4 sm:p-6 rounded-xl border-2 border-cosmos/20 shadow-lg hover:shadow-xl transition-shadow duration-300">
                  <img
                  ...
                  />
                  <p className="text-xs text-cosmos/60 mt-3 font-body">
                    Abra o app do seu banco e pague via PIX
                  </p>
                </div>
              ) : (
                <div className="bg-surface p-4 sm:p-6 rounded-xl border-2 border-cosmos/20 shadow-lg">
                  <p className="text-sm text-cosmos/70 font-body">
              ...
              {/* CÓDIGO COPIA E COLA */}
              {pixData.qrCode && (
                <div className="bg-elevated p-4 sm:p-5 rounded-xl border border-cosmos/10">
                  <p className="text-sm font-medium text-cosmos mb-3 font-body">
                    Código PIX Copia e Cola:
                  </p>
                  <div className="bg-surface p-3 rounded-lg border border-cosmos/20 mb-4 max-h-28 overflow-y-auto">
                    <p className="text-xs font-mono break-all text-cosmos/70">
                  ...
                  <Button
                    ...
                    className={`w-full rounded-lg transition-all duration-300 ${
                      copied
                        ? 'bg-fire hover:bg-fire-bright text-white'
                        : 'bg-cosmos hover:bg-cosmos/90 text-white'
                    }`}
                    size="lg"
          ...
          {isPix && !pixData && !pixLoading && (
            <Alert className="mb-6 border-cosmos/30 bg-cosmos/5 rounded-xl">
              <QrCode className="h-5 w-5 text-cosmos flex-shrink-0" />
              <AlertDescription className="text-cosmos text-left ml-2 font-body">
                {pixLoadFailed === 'RATE_LIMIT' ? (
                  ...
                  <>
                    ...
                    <br />
                    <span className="text-xs text-cosmos/70 mt-2 block">
          ...
          {isBoleto && (
            <Alert className="mb-6 border-cosmos/30 bg-cosmos/5 rounded-xl">
              <Barcode className="h-5 w-5 text-cosmos flex-shrink-0" />
              <AlertDescription className="text-cosmos text-left ml-2 font-body">
                  ...
                  <div className="mt-4 pt-4 border-t border-cosmos/10">
                    ...
                    <div className="space-y-2">
                      {order.items.map((item, idx) => (
                        <div key={idx} className="text-xs text-cosmos/60 font-body">
                          {item.quantity}x {item.name || `Produto ${item.productId.substring(0, 8)}`}
                          {item.size && <span className="text-cosmos/40"> — Tamanho: {item.size}</span>}
                          {item.color && <span className="text-cosmos/40"> — Cor: {item.color}</span>}
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {order.shipping.address1 && (
                  <div className="mt-4 pt-4 border-t border-cosmos/10">
                    <div className="flex items-start gap-3">
                      <div className="w-8 h-8 bg-cosmos/10 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5">
                        <MapPin className="w-4 h-4 text-cosmos" />
                      </div>
                      <div className="text-xs text-cosmos/70 font-body text-left">
                        <p className="font-medium text-cosmos">{order.shipping.address1}</p>
          ...
          <div className="mb-6 p-4 sm:p-5 bg-cosmos/5 rounded-xl border border-cosmos/10">
            <div className="flex items-center justify-center gap-2 mb-2">
              <div className="w-8 h-8 bg-cosmos/10 rounded-full flex items-center justify-center">
                <Mail className="w-4 h-4 text-cosmos" />
              </div>
              <p className="text-sm font-semibold text-cosmos font-body">Enviamos por email</p>
            </div>
            <p className="text-xs text-cosmos/60 font-body leading-relaxed">
          ...
          <div className="space-y-3">
            {(resolvedRef || order?.externalReference) ? (
              ...
              <p className="text-sm text-cosmos/60 py-2 font-body">
            ...
            <Link to="/" className="block">
              <Button
                variant="ghost"
                className="w-full text-cosmos/50 hover:text-cosmos rounded-lg transition-colors"
                size="sm"
              >
          ...
          {/* Footer */}
          <p className="text-xs text-cosmos/50 mt-6 font-body">
