...
export async function updateOrderPayment(req: Request, res: Response) {
  try {
    ...
    if (!externalReference) {
      return sendError(res, req, 400, 'VALIDATION_ERROR', 'externalReference is required');
    }

    const body = updatePaymentSchema.safeParse(req.body);
    if (!body.success) {
      return sendError(res, req, 400, 'VALIDATION_ERROR', 'Dados inválidos', { details: body.error.issues });
    }
    ...
    return sendError(res, req, 500, 'INTERNAL_ERROR', 'Erro ao atualizar pagamento');
  }
}
